...
- [`app/`](./app) — projeto Next.js 15 (App Router, TypeScript, Tailwind v4, shadcn/ui) integrado ao Supabase. Dono da migration canônica (`supabase/migrations/`, 27 tabelas, RLS por `auth.uid()`) e do motor de cálculo (`src/lib/calculo/`, funções puras com 39 testes unitários cobrindo a seção 5 do handoff). Ainda sem telas de produto.
