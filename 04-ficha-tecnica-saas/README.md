...
- [`app/`](./app) — projeto Next.js 15 (App Router, TypeScript, Tailwind v4, shadcn/ui) integrado ao Supabase. Dono da migration canônica (`supabase/migrations/`) com as 27 tabelas e RLS por `auth.uid()`. Ainda sem telas de produto.
- [`backend/`](./backend) — motor de cálculo das fórmulas de CMV, precificação, estoque e nutricional, como funções puras com 39 testes unitários.
